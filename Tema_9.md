...
### Результат.
![image](https://github.com/user-attachments/assets/c4a9a048-d48e-469e-b1f6-bb03abe2ce02)
...
### Результат.
![image](https://github.com/user-attachments/assets/d46ec43f-bee6-4b58-a1d0-8e668d1ad7b5)
...
### Результат.
![image](https://github.com/user-attachments/assets/877ac449-6baa-4a8a-999c-aa0d8dd743fc)
...
### Результат.
![image](https://github.com/user-attachments/assets/f58f5d2f-02ab-4764-9d30-58086c71bd67)
...
### Результат.
![image](https://github.com/user-attachments/assets/c9e3f2a6-0d04-4cb8-9c39-c9816d899904)
...
### Результат.
![image](https://github.com/user-attachments/assets/6360536c-329c-401d-80e2-4c1c92898893)
...
### Результат.
![image](https://github.com/user-attachments/assets/702c8164-0f3b-430f-837a-9289bfb8e2ca)
...
### Результат.
![image](https://github.com/user-attachments/assets/219827ed-c03b-4953-b6ee-2658c4f14fb1)
...
### Результат.
![image](https://github.com/user-attachments/assets/7bc93fb7-14ae-4c22-9afc-f913a03485f2)
...
### Результат.
![image](https://github.com/user-attachments/assets/549f610d-7c1f-4b00-9cff-75ccba6e9665)
...
## Вывод
1. `Tomato`:
  - `states`: Список всех стадий созревания томата.
  - `__init__(self, index)`: Инициализирует томат с заданным индексом и устанавливает начальное состояние "отсутствует".
  - `grow(self)`: Переводит томат на следующую стадию созревания.
  - `is_ripe(self)`: Проверяет, является ли томат спелым (красным).

2. `TomatoBush`:
  - `__init__(self, num_tomatoes)`: Создает список томатов на кусте, где `num_tomatoes` - количество томатов.
  - `grow_all(self)`: Переводит все томаты на кусте на следующую стадию созревания.
  - `all_are_ripe(self)`: Проверяет, все ли томаты на кусте спелые.
  - `give_away_all(self)`: Удаляет все томаты с куста после сбора урожая.

3. `Gardener`:
  - `__init__(self, name, plant)`: Инициализирует садовника с заданным именем и растением (`TomatoBush`).
  - `work(self)`: "Работает" садовник, позволяя всем томатам на кусте расти.
  - `harvest(self)`: Садовник собирает урожай, если все томаты созрели.
  - `knowledge_base()`: Статический метод, предоставляющий справку о садоводстве.

4. Тесты:
- Проводят симуляцию роста томатов и показывают, как садовник собирает урожай.

## Общие выводы по теме
Основные концепции ООП:

1. Объекты: Это фундаментальные элементы ООП, представляющие собой комбинацию данных (атрибутов) и поведения (методов). Например, в программе "Садовник и Помидоры" - `Tomato`, `TomatoBush` и `Gardener` - это объекты.
2. Классы: Это "чертежи" для создания объектов. Класс определяет атрибуты и методы, которые будут доступны всем объектам, созданным на основе этого класса.
3. Наследование: Позволяет создавать новые классы, наследуя свойства и методы от существующих. Это позволяет избежать дублирования кода и создавать иерархию классов. В примере с помидорами, классы `Dog` и `Cat` наследуют от `Mammal`.
4. Инкапсуляция: Скрывает реализацию данных и методов внутри класса, предоставляя доступ к ним только через публичные методы. Это помогает сделать код более структурированным, гибким и устойчивым к ошибкам. В примере, использование _ в начале имени атрибута делает его приватным, доступ к нему осуществляется через геттеры и сеттеры.
5. Полиморфизм: Позволяет использовать один и тот же интерфейс (методы) для объектов разных типов. Это делает код более универсальным и гибким. В примере с приветствием, метод `greeting()` может быть вызван как для `Russian`, так и для `English` классов.
