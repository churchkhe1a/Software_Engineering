...
2. `return max(lst) / len(lst)`: Внутри функции выполняется следующее:
  *  max(lst): Находит максимальное значение в списке lst.
  *  len(lst): Определяет длину списка lst.
  *  /: Максимальное значение делится на длину списка.
  *  return: Результат деления возвращается как значение функции.
